      ...
      <div class="bg-slate-900 border border-slate-800/50 rounded-xl shadow-sm overflow-hidden">
        <table class="min-w-full divide-y divide-slate-800/50">
          ...
          <tbody class="bg-slate-900 divide-y divide-slate-800/50">
            <tr *ngFor="let doc of doctors" class="hover:bg-slate-800/50 transition-colors">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                ...
                <button (click)="deleteDoctor(doc.doctor_id)" class="text-red-400 hover:text-red-300 px-2 py-1 rounded hover:bg-red-900/30 transition-colors"><fa-icon [icon]="faTrash"></fa-icon></button>
              </td>
            </tr>
      ...
      <div *ngIf="showModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 backdrop-blur-sm">
        <div class="bg-slate-900 rounded-xl shadow-2xl max-w-md w-full p-6 border border-slate-800/50">
          ...
          <form (ngSubmit)="saveDoctor()" class="space-y-4">
            <div>
              <label class="block text-sm font-semibold text-slate-200 mb-1">Doctor ID</label>
              <input name="id" [(ngModel)]="currentDoc.doctor_id" [disabled]="isEditing" required class="w-full px-4 py-2 border border-slate-600 rounded-lg bg-slate-800 text-white placeholder-slate-500 focus:ring-2 focus:ring-teal-500 focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed transition-colors sm:text-sm" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-slate-200 mb-1">Name</label>
              <input name="name" [(ngModel)]="currentDoc.doctor_name" required class="w-full px-4 py-2 border border-slate-600 rounded-lg bg-slate-800 text-white placeholder-slate-500 focus:ring-2 focus:ring-teal-500 focus:border-transparent sm:text-sm" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-slate-200 mb-1">Specialization</label>
              <input name="spec" [(ngModel)]="currentDoc.specialization" required class="w-full px-4 py-2 border border-slate-600 rounded-lg bg-slate-800 text-white placeholder-slate-500 focus:ring-2 focus:ring-teal-500 focus:border-transparent sm:text-sm" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-slate-200 mb-1">Email</label>
              <input name="email" type="email" [(ngModel)]="currentDoc.email" class="w-full px-4 py-2 border border-slate-600 rounded-lg bg-slate-800 text-white placeholder-slate-500 focus:ring-2 focus:ring-teal-500 focus:border-transparent sm:text-sm" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-slate-200 mb-1">Phone</label>
              <input name="phone" [(ngModel)]="currentDoc.phone" class="w-full px-4 py-2 border border-slate-600 rounded-lg bg-slate-800 text-white placeholder-slate-500 focus:ring-2 focus:ring-teal-500 focus:border-transparent sm:text-sm" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-slate-200 mb-1">Available Days</label>
              <input name="days" [(ngModel)]="currentDoc.available_days" placeholder="e.g. Mon, Wed, Fri" class="w-full px-4 py-2 border border-slate-600 rounded-lg bg-slate-800 text-white placeholder-slate-500 focus:ring-2 focus:ring-teal-500 focus:border-transparent sm:text-sm" />
            </div>

            <div class="pt-4 flex justify-end gap-3">
              <button type="button" (click)="closeModal()" class="bg-slate-800 py-2 px-4 border border-slate-600 rounded-lg shadow-sm text-sm font-medium text-slate-200 hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-teal-500 transition-colors">Cancel</button>
              <button type="submit" class="bg-teal-600 hover:bg-teal-700 py-2 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-teal-500 transition-colors">Save</button>
            </div>
          </form>
        </div>
      </div>
